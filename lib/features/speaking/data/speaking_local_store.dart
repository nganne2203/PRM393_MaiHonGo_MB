...
class SpeakingLocalStore {
  static const _pendingKey = 'speaking_pending_attempts';

  Future<List<PendingSpeakingAttempt>> loadPendingAttempts() async {
    ...
    return decoded
        .whereType<Map>()
        .map((item) => PendingSpeakingAttempt.fromJson(
              item.map((key, value) => MapEntry(key.toString(), value)),
            ))
        .toList();
  }

  Future<void> addPendingAttempt(PendingSpeakingAttempt attempt) async {
  ...
  Future<void> _save(List<PendingSpeakingAttempt> attempts) async {
    final prefs = await SharedPreferences.getInstance();
